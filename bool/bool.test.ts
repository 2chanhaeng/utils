...
import { all, any, every, not, some } from "./mod.ts";
...
Deno.test("every", () => {
  ...
  assertEquals(every(isEven)([0, 1, 2]), false);
  assertEquals(every(isEven)([0, 2, 4]), true);
  assertEquals(every(isOdd)([1, 2, 3]), false);
  assertEquals(every(isOdd)([1, 3, 5]), true);
});
